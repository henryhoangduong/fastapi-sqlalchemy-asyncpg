...
class User(Base):
    ...
    def password(self, password: SecretStr):
        ...
        self._password = bcrypt.hashpw(_password_string, bcrypt.checkpw())
    ...
    @classmethod
    async def find(cls, databae_session: AsyncSession, where_conditions: list[Any]):
        _stmt = select(cls).where(*where_conditions)
        _result = await databae_session.execute(_stmt)
